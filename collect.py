...
def collect(terminated_file):
    ...
    lines_lenght = int(lines_lenght_[0].split(' ')[0])
    ...
    for line_index in range(lines_lenght):
        ...
        system("curl -s -r{}-$(({}+{}-1)) https://commoncrawl.s3.amazonaws.com/{} > tmp.warc.gz".format(index["offset"], index["offset"], index["length"], index["filename"])) 
        system("warcio extract --payload tmp.warc.gz 0 > tmp.html")
        system("lynx --dump tmp.html > /content/text/tmp{}.txt".format(str(line_index)))
